# Hi, I'm Lindsey! 👋🏻

**Mobile Developer @ Webmotors | Flutter & Dart | Android & Kotlin**

I'm a Mobile Developer focused on building **scalable, performant and maintainable applications**.

Currently, I work at **Webmotors**, contributing to *Webmotors Serviços*, a product used by thousands of users for automotive services and vehicle debt management.


## 🛠️ Tech Stack
<div style="display: inline_block"><br>
  <img align="center" title="Flutter" alt="Flutter-icon" height="30" width="40" src="https://raw.githubusercontent.com/devicons/devicon/refs/heads/master/icons/flutter/flutter-original.svg">
  <img align="center" title="Dart" alt="Dart-icon" height="30" width="40" src="https://raw.githubusercontent.com/devicons/devicon/refs/heads/master/icons/dart/dart-original.svg">
  <img align="center" title="Kotlin" alt="Kotlin-icon" height="30" width="40" src="https://raw.githubusercontent.com/devicons/devicon/refs/heads/master/icons/kotlin/kotlin-original.svg">
  <img align="center" title="Android" alt="Android-icon" height="30" width="40" src="https://raw.githubusercontent.com/devicons/devicon/refs/heads/master/icons/android/android-original.svg">
  <img align="center" title="Firebase" alt="Firebase-icon" height="30" width="40" src="https://raw.githubusercontent.com/devicons/devicon/refs/heads/master/icons/firebase/firebase-original.svg">
  <img align="center" title="Firebase" alt="Firebase-icon" height="30" width="40" src="https://raw.githubusercontent.com/devicons/devicon/refs/heads/master/icons/datadog/datadog-original.svg">
</div>

### 🏗️ What I work with

- **Flutter & Dart** for cross-platform mobile development
- **Clean Architecture** for scalable and maintainable applications
- **Bloc/Cubit** for state management
- **Unit & Widget Tests** for application reliability
- **Android & Kotlin** and integration with native applications
- **Firebase & Datadog** for services and application monitoring
- Technical refinements, **Code Reviews** and collaboration with Product & Business teams

## 🎬 Featured Project

### Movie Search

Flutter application for searching movies using the **TMDB API**.

Built with:
`Flutter` · `Dart` · `Clean Architecture` · `BLoC` · `Infinite Pagination` · `Image Cache` · `Automated Tests`

[View project →](https://github.com/LindseyFontana/movie_search)

 ## 📫 Let's connect
<div> 
  <a href = "mailto:lindseyfontanas@gmail.com"><img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white" target="_blank"></a>
  <a href="https://www.linkedin.com/in/lindsey-fontana-schmitz/" target="_blank"><img src="https://img.shields.io/badge/-LinkedIn-%230077B5?style=for-the-badge&logo=linkedin&logoColor=white" target="_blank"></a>  
</div>
